...
public class LoggedTalon{
    ...
    public void logStats(){
        positionLogEntry.append(
            motor.getPosition().getValueAsDouble(), GRTUtils.getFPGATime()
        );

        veloLogEntry.append(
            motor.getVelocity().getValueAsDouble(), GRTUtils.getFPGATime()
        );

        appliedVoltsLogEntry.append(
            motor.getMotorVoltage().getValueAsDouble(), GRTUtils.getFPGATime()
        );

        supplyCurrLogEntry.append(
            motor.getSupplyCurrent().getValueAsDouble(), GRTUtils.getFPGATime()
        );

        statorCurrLogEntry.append(
            motor.getStatorCurrent().getValueAsDouble(), GRTUtils.getFPGATime()
        );

        temperatureLogEntry.append(
            motor.getDeviceTemp().getValueAsDouble(), GRTUtils.getFPGATime()
        );

        targetPositionLogEntry.append(targetPosition, GRTUtils.getFPGATime());

        targetVelocityLogEntry.append(targetVelocity, GRTUtils.getFPGATime());

        targetTorqueCurrentFOCLogEntry.append(
            targetTorqueCurrentFOC, GRTUtils.getFPGATime()
        );

        targetDutyCycleLogEntry.append(targetDutyCycle, GRTUtils.getFPGATime());

        closedLoopErrorLogEntry.append(
            motor.getClosedLoopError().getValueAsDouble(), GRTUtils.getFPGATime()
        );
    }
}